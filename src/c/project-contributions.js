                            ...
                            m('.w-col.w-col-11', [
                                m('.fontsize-base.fontweight-semibold', [
                                    m('a.link-hidden-dark[href="/users/' + contribution.user_id + '"]', contribution.user_name), (contribution.is_owner_or_admin ?
                                        m('.fontsize-smaller', [
                                            '$ ' + h.formatNumber(contribution.value, 2, 3), (contribution.anonymous ? [m.trust('&nbsp;-&nbsp;'), m('strong', 'Anonymous contribution')] : '')
                                        ]) : ''),
                                    m('.fontsize-smaller', h.momentify(contribution.created_at, 'DD/MM/YYYY, HH:mm') + 'h'),
                                    m('.fontsize-smaller', (contribution.total_contributed_projects > 1 ? 'Supported this and ' + contribution.total_contributed_projects + ' other projects.' : 'Only supports this project so far.'))
                                ])
                            ])
                        ]),
